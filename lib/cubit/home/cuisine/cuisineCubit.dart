...
class CuisineCubit extends Cubit<CuisineState> {
  ...
  Future<List<CuisineModel>> _fetchData({
    ...
    try {
      ...
      final result = await Api.post(
          body: body, url: Api.getCategoriesUrl, token: true, errorCode: false);
      totalHasMore = result[totalKey].toString();
      if (type == popularCategoriesKey) {
        return (result['popular_categories'] as List)
  ...
  void fetchMoreCuisineData(String limit, String type, String branchId) {
    _fetchData(
            limit: limit,
            offset: (state as CuisineSuccess).cuisineList.length.toString(),
            type: type,
            branchId: branchId)
        .then((value) {
      final oldState = (state as CuisineSuccess);
      final List<CuisineModel> usersDetails = value;
      final List<CuisineModel> updatedUserDetails =
          List.from(oldState.cuisineList);
      updatedUserDetails.addAll(usersDetails);
      emit(CuisineSuccess(updatedUserDetails, oldState.totalData,
  ...
  void cuisineInitialize() {
    return emit(CuisineFailure(StringsRes.somethingWentWrong));
  }
}
