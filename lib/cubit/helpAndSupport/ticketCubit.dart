...
class TicketCubit extends Cubit<TicketState> {
  ...
  Future<List<TicketModel>> getTicket({
    ...
    try {
      ...
      final result = await Api.post(
          body: body, url: Api.getTicketsUrl, token: true, errorCode: true);
  ...
  void fetchTicket(String limit, String userId) {
    ...
    getTicket(limit: limit, userId: userId).then((value) {
      ...
      emit(TicketFailure(ticketException.errorMessage.toString(),
          ticketException.errorStatusCode.toString()));
    });
  }
  ...
  void fetchMoreTicketData(String limit, String userId) {
    getTicket(
            limit: limit,
            offset: (state as TicketSuccess).ticketList.length.toString(),
            userId: userId)
        .then((value) {
      final oldState = (state as TicketSuccess);
      final List<TicketModel> usersDetails = value;
      final List<TicketModel> updatedUserDetails =
          List.from(oldState.ticketList);
      updatedUserDetails.addAll(usersDetails);
      emit(TicketSuccess(updatedUserDetails, oldState.totalData,
  ...
  void addTicket(TicketModel ticketModel) {
    if (state is TicketSuccess) {
      ...
      emit(TicketSuccess(
          List<TicketModel>.from(currentTicket), totalData, hasMore));
    }
  }
  ...
  void editTicket(TicketModel ticketModel) {
    if (state is TicketSuccess) {
      List<TicketModel> currentTicket = (state as TicketSuccess).ticketList;
      int i =
          currentTicket.indexWhere((element) => element.id == ticketModel.id);
      bool hasMore = (state as TicketSuccess).hasMore;
      int totalData = (state as TicketSuccess).totalData;
