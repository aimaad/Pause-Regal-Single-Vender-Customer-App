...
class GetProductRatingCubit extends Cubit<GetProductRatingState> {
  ...
  void fetchGetProductRating(String limit, String productId) {
    ...
    _fetchData(limit: limit, productId: productId).then((value) {
      ...
      emit(GetProductRatingFailure(ratingException.errorMessage.toString(),
          ratingException.errorStatusCode.toString()));
    });
  }
  ...
  void fetchMoreGetProductRatingData(String limit, String productId) {
    _fetchData(
            limit: limit,
            offset: (state as GetProductRatingSuccess)
                .productRatingList
                .length
                .toString(),
            productId: productId)
      ...
      final List<ProductRatingModel> usersDetails = value;
      final List<ProductRatingModel> updatedUserDetails =
          List.from(oldState.productRatingList);
      updatedUserDetails.addAll(usersDetails);
      emit(GetProductRatingSuccess(updatedUserDetails, oldState.totalData,
