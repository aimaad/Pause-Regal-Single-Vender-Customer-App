...
class GetRiderRatingCubit extends Cubit<GetRiderRatingState> {
  ...
  Future<List<RiderRatingModel>> _fetchData(
      {required String limit, String? offset, String? riderId}) async {
    try {
      ...
      final result = await Api.post(
          body: body, url: Api.getRiderRatingUrl, token: true, errorCode: true);
      totalHasMore = result[totalKey].toString();
      return (result[dataKey] as List)
          .map((e) => RiderRatingModel.fromJson(e))
          .toList();
    } catch (e) {
  ...
  void fetchMoreGetRiderRatingData(String limit, String riderId) {
    _fetchData(
            limit: limit,
            offset: (state as GetRiderRatingSuccess)
                .riderRatingList
                .length
                .toString(),
            riderId: riderId)
      ...
      final List<RiderRatingModel> usersDetails = value;
      final List<RiderRatingModel> updatedUserDetails =
          List.from(oldState.riderRatingList);
      updatedUserDetails.addAll(usersDetails);
      emit(GetRiderRatingSuccess(updatedUserDetails, oldState.totalData,
