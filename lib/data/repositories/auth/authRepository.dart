...
class AuthRepository {
  ...
  Future<Map<String, dynamic>> updateFcmId({String? fcmId}) async {
    final result = await _authRemoteDataSource.updateFcmId(fcmId: fcmId);
  ...
  Future<bool> deleteMyAccount() async {
    try {
      final result = await _authRemoteDataSource.deleteMyAccount();
      print(result);
      return result;
  ...
  Future<Map<String, dynamic>> signInUser(
      AuthProviders authProvider, String? referCode, String? friendCode) async {
    try {
      ...
      if (registeredUser[errorKey] == true) {
        ...
        await _authLocalDataSource
            .setCompany(registeredUser[dataKey]['company']);
        await _authLocalDataSource.setAddress(registeredUser['address']);
        await _authLocalDataSource
            .setCountryCode(registeredUser['country_code']);
        ...
        await _authLocalDataSource
            .setPinCode(registeredUser[dataKey]['pincode']);
        await _authLocalDataSource
            .setServiceableCity(registeredUser[dataKey]['serviceable_city']);
        await _authLocalDataSource
            .setReferralCode(registeredUser[dataKey]['referral_code']);
        await _authLocalDataSource
            .setFriendsCode(registeredUser[dataKey]['friends_code']);
        await _authLocalDataSource.setFcmId(registeredUser[dataKey]['fcm_id']);
        await _authLocalDataSource
            .setLatitude(registeredUser[dataKey]['latitude']);
        await _authLocalDataSource
            .setLongitude(registeredUser[dataKey]['longitude']);
        await AuthLocalDataSource.setJwtTocken(
